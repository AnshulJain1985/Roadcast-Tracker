...
public class JCX00ProtocolDecoder extends BaseProtocolDecoder {

    private static final int DATETIMECORRECTION = +330;
    private final Map<Integer, ByteBuf> photos = new HashMap<>();

    public JCX00ProtocolDecoder(JCX00Protocol protocol) {
        super(protocol);
    }
...
//        }
    ...
    private Object decodeJCXXAlarm(ByteBuf buf, DeviceSession deviceSession) {
        Position position = new Position(getProtocolName());
        position.setDeviceId(deviceSession.getDeviceId());
...
//        Log.info("JCX00 HEX: " + ByteBufUtil.hexDump(buf));
