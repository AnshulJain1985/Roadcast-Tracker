...
public class Rp05ProtocolEncoder extends BaseProtocolEncoder {

    public Rp05ProtocolEncoder(Protocol protocol) {
        super(protocol);
    }

    private ByteBuf encodeContent(long deviceId, String content) {

        boolean language = Context.getIdentityManager().lookupAttributeBoolean(deviceId, "rp05.language", false, false, true);
    ...
    protected Object encodeCommand(Command command) {
        ...
        boolean alternative = Context.getIdentityManager().lookupAttributeBoolean(
                command.getDeviceId(), "rp05.alternative", false, false, true);

        switch (command.getType()) {
            case Command.TYPE_ENGINE_STOP:
                return encodeContent(command.getDeviceId(), alternative ? "DYD#" : "DYD#");
            case Command.TYPE_ENGINE_RESUME:
                return encodeContent(command.getDeviceId(), alternative ? "HFYD#" : "HFYD#");
