...
public class S15AssetProtocolDecoder extends BaseProtocolDecoder {

    private final Map<Integer, ByteBuf> photos = new HashMap<>();

    public S15AssetProtocolDecoder(Protocol protocol) {
        super(protocol);
    }
...
//                break;
        ...
        int battery = buf.readUnsignedByte() * 100 / 6;
    ...
    private Object decodeBasic(Channel channel, SocketAddress remoteAddress, ByteBuf buf) {
        ...
        if (type == MSG_LOGIN) {
            ...
            if (buf.readableBytes() >= 2 + 6) {
                double batteryVoltage = buf.readUnsignedShort() * 0.01;
                position.set(Position.KEY_BATTERY, batteryVoltage);
                position.set(Position.KEY_BATTERY_LEVEL, calculateBatteryLevel(batteryVoltage));
            }
