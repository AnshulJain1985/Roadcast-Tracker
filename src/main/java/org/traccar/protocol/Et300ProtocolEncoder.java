...
public class Et300ProtocolEncoder extends BaseProtocolEncoder {

    public Et300ProtocolEncoder(Protocol protocol) {
        super(protocol);
    }

    private ByteBuf encodeContent(long deviceId, String content) {

        boolean language = Context.getIdentityManager().lookupAttributeBoolean(deviceId, "et300.language", false, false, true);
    ...
    protected Object encodeCommand(Command command) {
        ...
        boolean alternative = Context.getIdentityManager().lookupAttributeBoolean(
                command.getDeviceId(), "et300.alternative", false, false, true);

        switch (command.getType()) {
            case Command.TYPE_ENGINE_STOP:
                return encodeContent(command.getDeviceId(), alternative ? "DYD,000000#" : "DYD,000000#");
            case Command.TYPE_ENGINE_RESUME:
                return encodeContent(command.getDeviceId(), alternative ? "HFYD,000000#" : "HFYD,000000#");
