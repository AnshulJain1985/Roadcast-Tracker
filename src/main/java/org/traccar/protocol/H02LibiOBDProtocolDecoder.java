...
public class H02LibiOBDProtocolDecoder extends BaseProtocolDecoder {

    public H02LibiOBDProtocolDecoder(Protocol protocol) {
        super(protocol);
    }
    ...
    protected Object decode(
        ...
        boolean isMoving = position.getSpeed() > Context.getTripsConfig().getSpeedThreshold();
        position.set(Position.KEY_MOTION, isMoving);
        position.set(Position.KEY_IGNITION, isMoving);

        if (position.getLatitude() == 0 || position.getLongitude() == 0 || checkValidPosition(position)) {
