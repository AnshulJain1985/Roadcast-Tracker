...
public class BoltCamProtocolDecoder extends BaseProtocolDecoder {

    public BoltCamProtocolDecoder(BoltCamProtocol protocol) {
        super(protocol);
    }
    ...
    public static final int MSG_RTP_CAMERA_PLAY = 0x9101;
    public static final int MSG_RTP_CAMERA_PLAY_CONTROL = 0x9102;

    public static final int RESULT_SUCCESS = 0;

    public static ChannelBuffer formatMessage(int type, ChannelBuffer id, boolean shortIndex, ChannelBuffer data) {
    ...
    private void sendGeneralResponse(
        ...
        if (channel != null) {
            ...
            channel.write(formatMessage(MSG_GENERAL_RESPONSE, id, false, response), remoteAddress);
        }
    }

    private void sendGeneralResponse2(
            Channel channel, SocketAddress remoteAddress, ChannelBuffer id, int type) {
        if (channel != null) {
            ChannelBuffer response = ChannelBuffers.dynamicBuffer();
            response.writeShort(type);
            response.writeByte(RESULT_SUCCESS);
            channel.write(formatMessage(MSG_GENERAL_RESPONSE_2, id, true, response), remoteAddress);
        }
    }
    ...
    protected Object decode(
        ...
        ChannelBuffer id = buf.readSlice(6); // phone number
        int index;
        ...
        if (type == MSG_TERMINAL_REGISTER) {
            ...
            if (channel != null) {
                ...
                response.writeShort(index);
                response.writeByte(RESULT_SUCCESS);
                response.writeBytes(ChannelBuffers.hexDump(id).getBytes(StandardCharsets.US_ASCII));
                channel.write(formatMessage(MSG_TERMINAL_REGISTER_RESPONSE, id, false, response),
                        remoteAddress);
            }
    ...
    private Position decodeLocation(DeviceSession deviceSession, ChannelBuffer buf) {
        ...
        while (buf.readableBytes() > 2) {
            ...
            switch (subtype) {
                ...
                case 0x31:
                    ...
                    break;
                case 0x33:
                    String sentence = buf.readBytes(length).toString(StandardCharsets.US_ASCII);
    ...
    private Position decodeLocation2(DeviceSession deviceSession, ChannelBuffer buf, int type) {
    ...
    private List<Position> decodeLocationBatch(DeviceSession deviceSession, ChannelBuffer buf) {
