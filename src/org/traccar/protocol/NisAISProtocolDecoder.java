...
public class NisAISProtocolDecoder extends BaseProtocolDecoder {

    public NisAISProtocolDecoder(NisAISProtocol protocol) {
        super(protocol);
    }
    ...
    private static final Pattern PATTERN_EMERGENCY = new PatternBuilder()
            ...
            .number("(d+),")                        // low battery value
            .number("(d+),")                        // memroy percent
            .number("(d+),")                        // upload rate ignition on
            .number("(d+),")                        // upload rate ignition off
            .number("(d+),")                        // memroy percent
            .number("(d)(d)(d)(d),")                // digital Input 4
            .number("(d)(d),")                      // analog Output 2
            .text("*")
            .compile();
