...
public class AtlAISProtocolDecoder extends BaseProtocolDecoder {

    public AtlAISProtocolDecoder(AtlAISProtocol protocol) {
        super(protocol);
    }
...
//        OS: Over Speed Alert
    ...
    private Object decodeHeartbeat(Position position, Channel channel, SocketAddress remoteAddress, String sentence) {
        ...
        position.setDeviceId(deviceSession.getDeviceId());
        position.set(Position.KEY_BATTERY_LEVEL, parser.nextDouble(0.0));
        ...
        for (int i = 1; i <= 2; i++) {
            position.set(Position.PREFIX_OUT + i, parser.nextDouble(0.0));
        }
        return position;
    }
    ...
    private Object decodeEmergency(Position position, Channel channel, SocketAddress remoteAddress, String sentence) {
        ...
        if (packetType.equals("EMR")) {
            if (channel != null) {
                channel.write("$MSG,PANICRLS<6906>&");
            }
...
//        String checksum = parser.next();
    ...
    private Object decodeNormal(Position position, Channel channel, SocketAddress remoteAddress, String sentence) {
        ...
        String alarm = decodeAlarm(packetType);
...
//        String checksum = parser.next();
    ...
    protected Object decode(
        ...
        String header = sentence.substring(2, 4);

        Log.info(String.format("%08X", channel.getId()) + " - ALTAIS HEX: " + sentence);
